...
void push_matrix(int* queq, int index, int back){
	
}
void pop_matrix(int* queq, int back){
...
void que_arr(int** a, int num, int* versh, int size) {
	int back;
	int *queq = NULL;
	back = -1;
	versh[num] = 1;
	queq = (int*)malloc(sizeof(int)*(size));
	back++;
	queq[back] = num;
	while (back+1) {
		num = queq[0];
